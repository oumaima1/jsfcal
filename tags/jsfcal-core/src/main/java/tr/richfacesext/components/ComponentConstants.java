package tr.richfacesext.components;

/**
 * 
 * @author mert
 *
 */
public interface ComponentConstants {

	String FACES_PREFIX = "faces/";
	
	String RICHFACESEXT_RESOURCE_LOADER_VIEW_ID = ".componentsResourceLoader";
	
	String RICHFACES_RESOURCE_FOLDER = "/META-INF/assets";
	
	String SUBFOLDER_IMG 	= "img";
	String SUBFOLDER_JS 	= "js";
	String SUBFOLDER_CSS 	= "css";
	String SUBFOLDER_FLASH	= "flash";
	
	String ACCESSKEY_ATTR = "accesskey";
	String ALT_ATTR = "alt";
	String DIR_ATTR = "dir";
	String DISABLED_ATTR = "disabled";
	String LANG_ATTR = "lang";
	String MAXLENGTH_ATTR = "maxlength";
	String ONBLUR_ATTR = "onblur";
	String ONCHANGE_ATTR = "onchange";
	String ONCLICK_ATTR = "onclick";
	String ONDBLCLICK_ATTR = "ondblclick";
	String ONFOCUS_ATTR = "onfocus";
	String ONKEYDOWN_ATTR = "onkeydown";
	String ONKEYPRESS_ATTR = "onkeypress";
	String ONKEYUP_ATTR = "onkeyup";
	String ONMOUSEDOWN_ATTR = "onmousedown";
	String ONMOUSEMOVE_ATTR = "onmousemove";
	String ONMOUSEOUT_ATTR = "onmouseout";
	String ONMOUSEOVER_ATTR = "onmouseover";
	String ONMOUSEUP_ATTR = "onmouseup";
	String READONLY_ATTR = "readonly";
	String SIZE_ATTR = "size";
	String STYLE_ATTR = "style";
	String STYLECLASS_ATTR = "styleClass";
	String TABINDEX_ATTR = "tabindex";
	String TITLE_ATTR = "title";
	
	String[] HTML_INPUT_TEXT_ATTRIBUTES = {
			ACCESSKEY_ATTR,
			ALT_ATTR,
			DIR_ATTR,
			DISABLED_ATTR,
			LANG_ATTR,
			MAXLENGTH_ATTR,
			ONBLUR_ATTR,
			ONCHANGE_ATTR,
			ONCLICK_ATTR,
			ONDBLCLICK_ATTR,
			ONFOCUS_ATTR,
			ONKEYDOWN_ATTR,
			ONKEYPRESS_ATTR,
			ONKEYUP_ATTR,
			ONMOUSEDOWN_ATTR,
			ONMOUSEMOVE_ATTR,
			ONMOUSEOUT_ATTR,
			ONMOUSEOVER_ATTR,
			ONMOUSEUP_ATTR,
			READONLY_ATTR,
			SIZE_ATTR,
			STYLE_ATTR,
			STYLECLASS_ATTR,
			TABINDEX_ATTR,
			TITLE_ATTR
	};	
}