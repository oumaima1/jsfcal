		...
		this.each(function() {
			...
			if (bo) { // "button options"
				...
				if (bo == true || bo.prevMonth !== false) {
					if (r2l) {
						bNextMonth.click(prevMonth);
						buttons.append(bNextMonth);							
					}
					else {
						bPrevMonth.click(prevMonth);						
						buttons.append(bPrevMonth);
					}
				}
				if (bo == true || bo.nextMonth !== false) {
					if (r2l) {
						bPrevMonth.click(nextMonth);
						buttons.append(bPrevMonth);							
					}
					else {
						bNextMonth.click(nextMonth);						
						buttons.append(bNextMonth);
					}
